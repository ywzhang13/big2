// Unit test for 雙響 (2-winner priority rule) and 三響 (3-winner simultaneous).
// Runs applyWinnerOrderRule + calculateSettlement on crafted state objects
// to verify the filtering + payment logic without needing a real game flow.
...
const dir = mkdtempSync(join(tmpdir(), "mj-test-"));
const src = join(dir, "run.ts");
writeFileSync(src, TEST_SRC);

// Run with tsx via npx, with the project's tsconfig paths resolution.
// The test imports "../lib/mahjong/gameLogic" relative to project — so we
// place the file inside the project tests dir instead and run tsx on it.
const projectTestPath = "/Users/jay/Documents/開發/big2/tests/_run-mw.ts";
writeFileSync(projectTestPath, TEST_SRC);
const res = spawnSync("npx", ["tsx", projectTestPath], {
  cwd: "/Users/jay/Documents/開發/big2",
  stdio: "inherit",
});
